...
- **The identity of entities** that operate within the landscape. These can be separated into three groups: a) natural entities, b) machine entities, and c) legal entities. While the two former are represented by the `Identity` resource type, `OrgUnit` represents the latter.
